 ...
 *
 * Call this only when the list is actually being sent. The API stores rows these
 * rules would reject, because the write boundary compares raw names where
 * `toFeatureRows` trims them, and such a row must not block an edit that leaves
 * the list alone.
 */
