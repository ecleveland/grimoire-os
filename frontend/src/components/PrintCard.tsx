...
export default function PrintCard({ name, tag, children }: PrintCardProps) {
  return (
    <div
      ...
      className="w-[5in] h-[3in] overflow-hidden flex flex-col rounded-md border border-gray-400 bg-white text-gray-900 p-3 break-inside-avoid"
    >
      ...
      <div className="flex-1 min-h-0 overflow-hidden pt-1">{children}</div>
    </div>
  );
}
