  ...
  describe('transformBackground', () => {
    it('maps API background to Prisma shape', () => {
      const api = {
        ...
        personality_traits: { from: { options: [{ string: 'I am kind.', option_type: 'string' }] } },
        ideals: { from: { options: [{ desc: 'Charity is important.', option_type: 'ideal' }] } },
        bonds: { from: { options: [{ string: 'I serve my temple.', option_type: 'string' }] } },
        flaws: { from: { options: [{ string: 'I am inflexible.', option_type: 'string' }] } },
      };
  ...
  describe('transformFeat', () => {
    it('maps API feat to Prisma shape', () => {
      const api = {
        ...
        prerequisites: [
          { ability_score: { name: 'STR' }, minimum_score: 13, type: 'ability_score' },
        ],
      };
      ...
      expect(result.description).toBe('You have advantage on attack rolls.\n\nYou can restrain a creature.');
