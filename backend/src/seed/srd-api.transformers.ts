...
export function transformEquipment(api: ApiEquipment) {
  ...
  const category =
    api.category_range ?? api.armor_category ?? api.equipment_category?.name ?? 'Adventuring Gear';
  const desc = [...(api.desc ?? []), ...(api.special ?? [])].join('\n\n') || null;
...
export function transformBackground(api: ApiBackground) {
  ...
  const toolProficiencies =
    api.starting_proficiencies?.filter(p => !p.name.startsWith('Skill: ')).map(p => p.name) ?? [];
